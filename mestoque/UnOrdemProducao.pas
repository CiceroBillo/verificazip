...
Type TRBFuncoesOrdemProducao = class(TRBLocalizaOrdemProducao)
  private
    ...
    function GravaDImpressaoConsumoFracao(VpaCodFilial,VpaSeqOrdem, VpaSeqFracao,VpaSeqProdutoFracao, VpaCodCorFracao : Integer;VpaConsumo : TList):string;
    function GravaDRevisaoFracaoFaccionistaTerceiro(VpaDRevisaoFracao : TRBDRevisaoFracaoFaccionista) : string;
    ...
    procedure RenumeraFracao(VpaDOrdemProducao : TRBDOrdemProducao;VpaSeqFracao : Integer);
    ...
    function RPesosCalculadoOrdemCorte(VpaDPlanoCorte : TRBDPlanoCorteCorpo):double;
    function AtualizaValPesoMateriaPrimaPlanoCorte(VpaDPlanoCorte : TRBDPlanoCorteCorpo; VpaDMateriaPrima : TRBDProduto;VpaPerDiferenca : Double):string;
    function AtualizaCustoConsumoFracoes(VpaDPlanoCorte : TRBDPlanoCorteCorpo) :string;
//    procedure AlteraQtdReservadaFracaoOPConsumo(VpaDOP : TRBDordemProducao;VpaDFracao : TRBDFracaoOP;VpaDConsumo : TRBDConsumoFracaoOP
  public
    ...
    function RQtdProdutosOP(VpaCodFilial, VpaSeqOrdem, VpaSeqProduto : Integer) : integer;
    ...
    function RNomProdutoFracao(VpaCodFilial,VpaSeqOrdem, VpaSeqFracao : Integer):string;
    function RCodClienteOP(VpaCodFilial,VpaSeqOrdem : Integer) : Integer;
    function RCodProjetoOP(VpaCodFilial, VpaSeqOrdem : Integer) : Integer;
    function RProdutoFracaoOP(VpaCodFilial,VpaSeqOrdem,VpaSeqFracao : Integer):Integer;
    function RCorFracaoOP(VpaCodFilial,VpaSeqOrdem,VpaSeqFracao : Integer):Integer;
    function RValorEstagio(VpaCodEstagio : Integer) : double;
    ...
    function EstornaRevisaoRetornoFracao(VpaCodFaccionista, VpaSeqRetorno, VpaSeqItem : Integer):string;
    ...
    function MarcaEstagioComoTerceirizado(VpaCodFilial,VpaSeqOrdem,VpaSeqFracao, VpaCodEstagio : Integer):string;
    procedure CalculaTotaisRevisaoTerceiro(VpaDRevisao : TRBDRevisaoFracaoFaccionista);
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RQtdProdutosOP(VpaCodFilial, VpaSeqOrdem, VpaSeqProduto: Integer): integer;
begin
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RPesosCalculadoOrdemCorte(VpaDPlanoCorte: TRBDPlanoCorteCorpo): double;
var
  VpfLacoProduto, VpfLacoFracao : Integer;
  VpfDItem : TRBDPlanoCorteItem;
  VpfDFracao : TRBDPlanoCorteFracao;
  VpfLargura, VpfAltura, VpfVolume : Double;
begin
  result := 0;
  for VpfLacoProduto := 0 to VpaDPlanoCorte.Itens.Count - 1 do
  begin
    VpfDItem := TRBDPlanoCorteItem(VpaDPlanoCorte.Itens.Items[VpfLacoProduto]);
    for VpfLacoFracao := 0 to VpfDItem.Fracoes.Count - 1 do
    begin
      VpfDFracao := TRBDPlanoCorteFracao(VpfDItem.Fracoes.Items[VpfLacoFracao]);
      AdicionaSQLAbreTabela(OrdAux,'select ALTMOLDE, LARMOLDE from fracaoopconsumo ' +
                                   '  where codfilial = ' +IntToStr(VpfDFracao.CodFilial)+
                                   '  and seqordem =  ' +IntToStr(VpfDFracao.SeqOrdem)+
                                   ' and seqfracao = ' +IntToStr(VpfDFracao.SeqFracao)+
                                   ' and SEQPRODUTO = ' +IntToStr(VpaDPlanoCorte.SeqMateriaPrima));
      while not OrdAux.Eof do
      begin
        VpfLargura := OrdAux.FieldByName('LARMOLDE').AsFloat;
        VpfAltura := OrdAux.FieldByName('ALTMOLDE').AsFloat;
        if config.ConverterMTeCMparaMM then
        begin
          VpfLargura := VpfLargura * 10;
          VpfAltura := VpfAltura * 10;
        end;
        VpfVolume := VpfLargura * VpfAltura*VpaDPlanoCorte.EspessuraChapaMP;
        VpfDFracao.PesProduto :=  VpfVolume * (((VpaDPlanoCorte.DensidadeVolumetricaMP /1000)/1000)/1000) ;
        result := result + VpfDFracao.PesProduto;
        OrdAux.Next;
      end;
      OrdAux.Close;
    end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RProdutoFracaoOP(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao: Integer): Integer;
begin
  AdicionaSQLAbreTabela(OrdAux,'Select SEQPRODUTO from FRACAOOP  ' +
                               ' Where CODFILIAL = ' + IntToStr(VpaCodFilial)+
                               ' AND SEQORDEM = ' + IntToStr(VpaSeqOrdem)+
                               ' AND SEQFRACAO = ' +IntToStr(VpaSeqFracao));
  result := OrdAux.FieldByName('SEQPRODUTO').AsInteger;
  OrdAux.Close;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RCodClienteOP(VpaCodFilial, VpaSeqOrdem: Integer): Integer;
begin
  AdicionaSQLAbreTabela(OrdAux,'Select CODCLI FROM ORDEMPRODUCAOCORPO ' +
                               ' Where EMPFIL = '+IntToStr(VpaCodFilial)+
                               ' AND SEQORD = ' +IntToStr(VpaSeqOrdem));
  result := OrdAux.FieldByName('CODCLI').AsInteger;
  OrdAux.Close;
end;

{******************************************************************************}
function TRBFuncoesOrdemProducao.RCodProjetoOP(VpaCodFilial, VpaSeqOrdem: Integer): Integer;
begin
  AdicionaSQLAbreTabela(OrdAux,'SELECT CODPRJ FROM ORDEMPRODUCAOCORPO ' +
                               ' Where EMPFIL = ' +IntToStr(VpaCodFilial)+
                               ' AND SEQORD = ' +IntToStr(VpaSeqOrdem));
  result := OrdAux.FieldByName('CODPRJ').AsInteger;
  OrdAux.Close;
end;
...
begin
  result := nil;
  if varia.TipoOrdemProducao in [toAgrupada] then
  begin
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RCorFracaoOP(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao: Integer): Integer;
begin
  AdicionaSQLAbreTabela(OrdAux,'Select CODCOR from FRACAOOP  ' +
                               ' Where CODFILIAL = ' + IntToStr(VpaCodFilial)+
                               ' AND SEQORDEM = ' + IntToStr(VpaSeqOrdem)+
                               ' AND SEQFRACAO = ' +IntToStr(VpaSeqFracao));
  result := OrdAux.FieldByName('CODCOR').AsInteger;
  OrdAux.Close;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          VpfDOrdemCorteItem.QtdMetrosProduto := VpfDOrdemCorteItem.QtdMetrosProduto + VpfDConsumo.QtdABaixar;
        end
        else
...
begin
  result := '';
  if VpaApagarOPAntes and
    (VpaDOrdemProducao.SeqOrdemaAdicionar = 0) then //se esta adicionado as fracoes em uma op existente nao pode apagar as fracoes que ja existem
  begin
...
begin
  ...
  begin
    ...
    if VpfDEstagio.IndTerceirizado then
      OrdCadastro.FieldByName('INDTERCEIRIZADO').AsString := 'S'
    else
      OrdCadastro.FieldByName('INDTERCEIRIZADO').AsString := 'N';
...
begin
  ...
  begin
    ...
    if VpfDCorteItem.QtdMetrosProduto <> 0 then
      OrdCadastro.FieldByname('QTDMETROPRODUTO').AsFloat := VpfDCorteItem.QtdMetrosProduto;
...
begin
  ...
  AdicionaSQLAbreTabela(OrdCadastro,'Select * from ORDEMSERRA '+
                                    ' WHERE CODFILIAL = 0 AND SEQORDEMPRODUCAO = 0 AND SEQORDEMSERRA = 0' );
  for VpfLaco := 0 to VpaDOrdemProducao.OrdensSerra.Count - 1 do
  begin
    VpfDOrdemSerra := TRBDOrdemSerra(VpaDOrdemProducao.OrdensSerra.Items[VpfLaco]);
    OrdCadastro.Insert;
  ...
  OrdCadastro.close;
  if result = '' then
    result := GravaDOrdemSerraFracao(VpaDOrdemProducao);
end;
...
begin
  ...
  AdicionaSQLAbreTabela(OrdCadastro,'Select * from ORDEMSERRAFRACAO '+
                                    ' WHERE CODFILIAL = 0 AND SEQORDEMPRODUCAO = 0 AND SEQORDEMSERRA = 0 AND SEQFRACAO = 0 ');
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.GravaDImpressaoConsumoFracao(VpaCodFilial,VpaSeqOrdem, VpaSeqFracao,VpaSeqProdutoFracao, VpaCodCorFracao : Integer;VpaConsumo : TList):string;
var
  VpfLaco, VpfSeqImpressao : Integer;
  VpfDConsumo : TRBDConsumoFracaoOP;
  VpfDesMaterialExtra : string;
begin
  result := '';
  ...
  begin
    ...
    if VpfDConsumo.IndMaterialExtra then
      VpfDesMaterialExtra := 'S'
    else
      VpfDesMaterialExtra := 'N';
    AdicionaSQLAbreTabela(OrdCadastro,'Select * from IMPRESSAOCONSUMOFRACAO ' +
                                      ' Where SEQPRODUTO = '+IntToStr(VpaSeqProdutoFracao)+
                                      ' AND CODCOR = ' +IntToStr(VpaCodCorFracao)+
                                      ' AND INDMATERIALEXTRA = '''+VpfDesMaterialExtra+''''+
                                      ' AND SEQMATERIAPRIMA = '+IntToStr(VpfDConsumo.SeqProduto)+
                                      ' AND CODCOR = '+IntToStr(VpfDConsumo.CodCor));
    if OrdCadastro.Eof then
    begin
      OrdCadastro.insert;
    ...
    OrdCadastro.FieldByName('SEQPRODUTO').AsInteger := VpaSeqProdutoFracao;
    if varia.TipoOrdemProducao in [toFracionada] then
    begin
      OrdCadastro.FieldByName('CODCOR').AsInteger := VpfDConsumo.CodCor;
      OrdCadastro.FieldByName('DESUM').AsString := VpfDConsumo.DesUM;
    end
    else
      OrdCadastro.FieldByName('CODCOR').AsInteger := VpaCodCorFracao;
    OrdCadastro.FieldByName('QTDPRODUTO').AsFloat := OrdCadastro.FieldByName('QTDPRODUTO').AsFloat + FunProdutos.CalculaQdadePadrao(VpfDConsumo.DesUM,OrdCadastro.FieldByName('DESUM').AsString,VpfDConsumo.QtdProduto,IntToStr( VpfDConsumo.SeqProduto));
    OrdCadastro.FieldByName('QTDBAIXADO').AsFloat := OrdCadastro.FieldByName('QTDBAIXADO').AsFloat + FunProdutos.CalculaQdadePadrao(VpfDConsumo.DesUM,OrdCadastro.FieldByName('DESUM').AsString,VpfDConsumo.QtdBaixado,IntToStr(VpfDConsumo.SeqProduto));
    OrdCadastro.FieldByName('QTDRESERVADA').AsFloat := OrdCadastro.FieldByName('QTDRESERVADA').AsFloat + FunProdutos.CalculaQdadePadrao(VpfDConsumo.DesUM,OrdCadastro.FieldByName('DESUM').AsString,VpfDConsumo.QtdReservado,IntToStr(VpfDConsumo.SeqProduto));
    OrdCadastro.FieldByName('QTDARESERVAR').AsFloat := OrdCadastro.FieldByName('QTDARESERVAR').AsFloat + FunProdutos.CalculaQdadePadrao(VpfDConsumo.DesUM,OrdCadastro.FieldByName('DESUM').AsString,VpfDConsumo.QtdAReservar,IntToStr(VpfDConsumo.SeqProduto));
    OrdCadastro.FieldByName('DESLOCALIZACAO').AsString :=  VpfDConsumo.DesLocalizacaoProduto;
    OrdCadastro.FieldByName('INDMATERIALEXTRA').AsString := VpfDesMaterialExtra;
    OrdCadastro.post;
    result := OrdCadastro.AMensagemErroGravacao;
    if OrdCadastro.AErronaGravacao then
      break;
...
begin
  ...
  if VpaDOrdem.NomCor <> '' then
    VpaTabela.FieldByName('DESCOR').AsString := VpaDOrdem.NomCor
  else
    VpaTabela.FieldByName('DESCOR').clear;

  if VpaDOrdem.CodTransportadora <> 0 then
    VpaTabela.FieldByName('CODTRA').AsInteger := VpaDOrdem.CodTransportadora
  else
    VpaTabela.FieldByName('CODTRA').Clear;
  ...
  if VpaDOrdem.CodProjeto <> 0 then
    VpaTabela.FieldByName('CODPRJ').AsInteger := VpaDOrdem.CodProjeto
  else
    VpaTabela.FieldByName('CODPRJ').Clear;
...
begin
  ...
  VpaDOrdem.CodProjeto := VpaTabela.FieldByName('CODPRJ').AsInteger;
...
begin
  ...
  AdicionaSQLAbreTabela(OrdTabela,'Select OPE.SEQESTAGIO, OPE.QTDCELULA, OPE.DESHORAS, OPE.QTDPRODUZIDO, '+
                        ' OPE.INDPRODUCAO, OPE.VALUNITARIOFACCAO, OPE.INDFINALIZADO, OPE.INDTERCEIRIZADO, ' +
                        ' OPE.CODESTAGIO, '+
                        ' EST.NOMEST '+
...
//    VpfDEstagioFracao.NumOrdem := OrdTabela.FieldByName('NUMORDEM').AsInteger;
    ...
    VpfDEstagioFracao.IndTerceirizado := OrdTabela.FieldByName('INDTERCEIRIZADO').AsString = 'S';
    OrdTabela.Next;
  end;
  ordTabela.close;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.AtualizaCustoConsumoFracoes(VpaDPlanoCorte: TRBDPlanoCorteCorpo): string;
var
  VpfPesoCalculado, VpfPesoReal, vpfPerDiferenca : Double;
  VpfDMateriaPrima : TRBDProduto;
begin
  result := '';
  VpfPesoCalculado := RPesosCalculadoOrdemCorte(VpaDPlanoCorte);
  if VpfPesoCalculado > 0 then
  begin
    if VpaDPlanoCorte.PerChapa <> 0 then
    begin
      VpfPesoReal := (VpaDPlanoCorte.PesoChapa * VpaDPlanoCorte.PerChapa) /100;
      vpfPerDiferenca :=((VpfPesoReal *100)/VpfPesoCalculado) - 100 ;
    end
    else
      vpfPerDiferenca := 0;

    VpfDMateriaPrima := TRBDProduto.Cria;
    FunProdutos.CarDProduto(VpfDMateriaPrima,varia.CodigoEmpresa,VpaDPlanoCorte.CodFilial,VpaDPlanoCorte.SeqMateriaPrima);
    result := AtualizaValPesoMateriaPrimaPlanoCorte(VpaDPlanoCorte,VpfDMateriaPrima,vpfPerDiferenca);
    VpfDMateriaPrima.Free;
  end;
...
Var
  VpfQtdRetorno : Double;
begin
  result := '';
  if config.PermitirEnviarFracaoMaisde1VezparaoMesmoFaccionista then
  begin
    LimpaSQLTabela(OrdCadastro);
    AdicionaSQLTabela(OrdCadastro,'Select * from FRACAOOPFACCIONISTA '+
                                      ' Where CODFILIAL = ' +IntToStr(VpaDRetornoFracao.CodFilial)+
                                      ' and SEQORDEM = ' +IntToStr(VpaDRetornoFracao.SeqOrdem)+
                                      ' and SEQFRACAO = ' +IntToStr(VpaDRetornoFracao.SeqFracao)+
                                      ' and CODFACCIONISTA = '+IntToStr(VpaDRetornoFracao.CodFaccionista)+
                                      ' AND DATFINALIZACAO IS NULL' );
    if VpaDRetornoFracao.SeqIDEstagio = 0 then
      OrdCadastro.sql.add(' and SEQESTAGIO IS NULL')
    else
      OrdCadastro.Sql.Add(' and SEQESTAGIO = ' +IntToStr(VpaDRetornoFracao.SeqIDEstagio));
    OrdCadastro.SQL.Add('ORDER BY DATCADASTRO');
    OrdCadastro.Open;
  end
  else
  ...
  VpfQtdRetorno := VpaDRetornoFracao.QtdRetorno;
  while not OrdCadastro.Eof and (VpfQtdRetorno > 0) do
  begin;
    OrdCadastro.Edit;
    if VpfQtdRetorno > (OrdCadastro.FieldByname('QTDENVIADO').AsFloat -  OrdCadastro.FieldByname('QTDDEVOLUCAO').AsFloat - OrdCadastro.FieldByname('QTDPRODUZIDO').AsFloat) then
    begin
      VpfQtdRetorno := VpfQtdRetorno - (OrdCadastro.FieldByname('QTDENVIADO').AsFloat - OrdCadastro.FieldByname('QTDPRODUZIDO').AsFloat - OrdCadastro.FieldByname('QTDDEVOLUCAO').AsFloat);
      OrdCadastro.FieldByname('QTDPRODUZIDO').AsFloat := OrdCadastro.FieldByname('QTDENVIADO').AsFloat - OrdCadastro.FieldByname('QTDDEVOLUCAO').AsFloat;
      OrdCadastro.FieldByname('DATFINALIZACAO').AsDateTime := now;
    end
    else
    begin
      OrdCadastro.FieldByname('QTDPRODUZIDO').AsFloat := OrdCadastro.FieldByname('QTDPRODUZIDO').AsFloat + VpfQtdRetorno;
      VpfQtdRetorno := 0;
    end;
  ...
  if VpfQtdRetorno > 0 then
  begin
    if OrdCadastro.FieldByName('CODFILIAL').AsInteger <> 0 then
    begin
      OrdCadastro.Edit;
      OrdCadastro.FieldByName('QTDPRODUZIDO').AsFloat := OrdCadastro.FieldByName('QTDPRODUZIDO').AsFloat + VpfQtdRetorno;
      OrdCadastro.Post;
...
begin
  ...
  begin
    LimpaSQLTabela(OrdCadastro);
    AdicionaSQLTabela(OrdCadastro,'Select * from FRACAOOPFACCIONISTA '+
                                      ' Where CODFILIAL = ' +IntToStr(VpaDDevolucaoFracao.CodFilial)+
                                      ' and SEQORDEM = ' +IntToStr(VpaDDevolucaoFracao.SeqOrdem)+
                                      ' and SEQFRACAO = ' +IntToStr(VpaDDevolucaoFracao.SeqFracao)+
                                      ' and CODFACCIONISTA = '+IntToStr(VpaDDevolucaoFracao.CodFaccionista)+
                                      ' AND DATFINALIZACAO IS NULL' );
    if VpaDDevolucaoFracao.SeqIDEstagio = 0 then
      OrdCadastro.sql.add(' and SEQESTAGIO IS NULL')
    else
      OrdCadastro.Sql.Add(' and SEQESTAGIO = ' +IntToStr(VpaDDevolucaoFracao.SeqIDEstagio));
    OrdCadastro.SQL.Add('ORDER BY DATCADASTRO');
    OrdCadastro.Open;
  end
  else
  ...
  VpfQtdDevolvido := VpaDDevolucaoFracao.QtdDevolvido;
  ...
  begin;
    ...
    begin
      OrdCadastro.FieldByname('QTDDEVOLUCAO').AsFloat := OrdCadastro.FieldByname('QTDDEVOLUCAO').AsFloat + VpfQtdDevolvido;
      VpfQtdDevolvido := 0;
    end;
    OrdCadastro.post;
    result := OrdCadastro.AMensagemErroGravacao;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.MarcaEstagioComoTerceirizado(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao,VpaCodEstagio: Integer): string;
begin
  ExecutaComandoSql(OrdCadastro,'UPDATE FRACAOOPESTAGIO ' +
                                    ' SET INDTERCEIRIZADO = ''S'''+
                                    ' Where CODFILIAL = '+IntToStr(VpaCodFilial)+
                                    ' and SEQORDEM = '+IntToStr(VpaSeqOrdem)+
                                    ' and SEQFRACAO = '+IntToStr(VpaSeqFracao)+
                                    ' and CODESTAGIO = '+IntToStr(VpaCodEstagio));
end;
...
begin
  result := '';
  if (VpaDOrdem.SeqOrdemaAdicionar = 0) then //se estiver adicionando uma submontagem na op atual não pode gravar a OP novamente, grava somente as frações da ordem de produção.
  begin
...
begin
  ...
  OrdCadastro.FieldByName('DATDIGITACAO').AsDateTime := VpaDFracaoFaccionista.DatDigitacao;
...
begin
  ...
  OrdCadastro.FieldByname('DATDIGITACAO').AsDateTime  := VpaDRetornoFracao.DatDigitacao;
  OrdCadastro.FieldByname('CODUSUARIO').AsInteger := VpaDRetornoFracao.CodUsuario;
  OrdCadastro.FieldByname('INDREVISADO').AsString := 'N';
  if config.RetornoFaccionistaRevisarAutomaticamente then
  begin
    OrdCadastro.FieldByname('QTDREVISADO').AsFloat := VpaDRetornoFracao.QtdRetorno;
    OrdCadastro.FieldByname('QTDDEFEITO').AsFloat := 0;
    OrdCadastro.FieldByname('DATREVISADO').AsDateTime := VpaDRetornoFracao.DatCadastro;
    OrdCadastro.FieldByname('INDREVISADO').AsString := 'S';
    OrdCadastro.FieldByname('CODUSUARIOREVISAO').AsFloat := VpaDRetornoFracao.CodUsuario;
  end;
  if VpaDRetornoFracao.IndFinalizar then
...
begin
  ...
  OrdCadastro.close;
  if result = '' then
    result := GravaDRevisaoFracaoFaccionistaTerceiro(VpaDRevisaoFracao);
end;

{******************************************************************************}
function TRBFuncoesOrdemProducao.GravaDRevisaoFracaoFaccionistaTerceiro(VpaDRevisaoFracao: TRBDRevisaoFracaoFaccionista): string;
var
  VpfLaco : Integer;
  VpfDTerceiro : TRBDRevisaoFracaoTerceiroFaccionista;
begin
  result := '';
  AdicionaSQLAbreTabela(OrdCadastro,'Select * from RETORNOFRACAOOPFACTERCEIRO '+
                                    ' Where CODFACCIONISTA = 0 AND SEQITEM = 0 AND SEQRETORNO = 0 AND SEQTERCEIRO = 0');
  for Vpflaco := 0 to VpaDRevisaoFracao.Terceiros.Count - 1 do
  begin
    VpfDTerceiro := TRBDRevisaoFracaoTerceiroFaccionista(VpaDRevisaoFracao.Terceiros.Items[VpfLaco]);
    ordCadastro.insert;
    OrdCadastro.FieldByName('CODFACCIONISTA').AsInteger := VpaDRevisaoFracao.CodFaccionista;
    OrdCadastro.FieldByName('SEQITEM').AsInteger := VpaDRevisaoFracao.SeqItem;
    OrdCadastro.FieldByName('SEQRETORNO').AsInteger := VpaDRevisaoFracao.SEqRetorno;
    OrdCadastro.FieldByName('SEQTERCEIRO').AsInteger := VpfLaco +1;
    OrdCadastro.FieldByName('QTDREVISADO').AsInteger := VpfDTerceiro.QtdRevisado;
    OrdCadastro.FieldByName('QTDDEFEITO').AsInteger := VpfDTerceiro.QtdDefeito;
    OrdCadastro.FieldByName('NOMTERCEIRO').AsString := UpperCase(VpfDTerceiro.NomTerceiro);
    OrdCadastro.Post;
    result := OrdCadastro.AMensagemErroGravacao;
    if result <> '' then
    break;
...
begin
  ...
  OrdCadastro.FieldByname('SEQCHAPA').AsInteger := VpaDPlanoCorte.SeqChapa;
  OrdCadastro.FieldByname('PERCHAPA').AsFloat := VpaDPlanoCorte.PerChapa;
  ...
  begin
    ...
    begin
      ...
      if result = '' then
      begin
        result := AtualizaCustoConsumoFracoes(VpaDPlanoCorte);
        if result = '' then
        begin
          result := FunProdutos.AtualizaEstoqueChapa(VpaDPlanoCorte);
        end;
      end;
    end;
  end;
...
var
  VpfLaco : Integer;
begin
  if VpaSeqFracaoPaiAtual <> VpaSeqFracaoPaiNovo then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (VpfDProCotacao.QtdProduto - VpfDProCotacao.QtdBaixadoNota > 0) and
      ...
      begin
        ...
        begin
          ...
          VpfDGerarOp.QtdProduto := VpfDProCotacao.QtdProduto - VpfDProCotacao.QtdBaixadoNota;
        ...
        begin
          VpfDGerarOp.QtdProduto := VpfDGerarOp.QtdProduto + FunProdutos.CalculaQdadePadrao(VpfDProCotacao.UM, VpfDGerarOp.UM,VpfDProCotacao.QtdProduto - VpfDProCotacao.QtdBaixadoNota ,IntToStr(VpfDProCotacao.SeqProduto));
  ...
  if VpaProdutos.Count = 1 then
    TRBDGerarOp(VpaProdutos.Items[0]).IndGerar := true;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RNomProdutoFracao(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao: Integer): string;
begin
  AdicionaSQLAbreTabela(OrdAux,'Select PRO.C_COD_PRO, PRO.C_NOM_PRO ' +
                               ' From CADPRODUTOS PRO, FRACAOOP FRA ' +
                               ' Where FRA.CODFILIAL = '+IntToStr(VpaCodFilial)+
                               ' AND FRA.SEQORDEM = '+IntToStr(VpaSeqOrdem)+
                               ' AND FRA.SEQFRACAO = '+IntToStr(VpaSeqFracao)+
                               ' AND FRA.SEQPRODUTO = PRO.I_SEQ_PRO ');
  result := OrdAux.FieldByName('C_COD_PRO').AsString + '-'+OrdAux.FieldByName('C_NOM_PRO').AsString;
  OrdAux.Close;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.EstornaRevisaoRetornoFracao(VpaCodFaccionista, VpaSeqRetorno, VpaSeqItem: Integer):string;
begin
  result := '';
  AdicionaSQLAbreTabela(OrdCadastro,'Select * from RETORNOFRACAOOPFACCIONISTA '+
                                    ' Where CODFACCIONISTA = '+IntToStr(VpaCodFaccionista)+
                                    ...
                                    ' AND SEQRETORNO = '+IntToStr(VpaSEqRetorno));
  OrdCadastro.edit;
  OrdCadastro.FieldByname('QTDREVISADO').clear;
  OrdCadastro.FieldByname('QTDDEFEITO').clear;
  OrdCadastro.FieldByname('DATREVISADO').clear;
  OrdCadastro.FieldByname('INDREVISADO').AsString := 'N';
  OrdCadastro.FieldByname('CODUSUARIOREVISAO').Clear;
  OrdCadastro.post;
  result := OrdCadastro.AMensagemErroGravacao;
  OrdCadastro.close;
  if result = '' then
  begin
    ExecutaComandoSql(OrdCadastro,'DELETE RETORNOFRACAOOPFACTERCEIRO '+
                                    ' Where CODFACCIONISTA = '+IntToStr(VpaCodFaccionista)+
                                    ' AND SEQITEM = '+IntToStr(VpaSeqItem)+
                                    ' AND SEQRETORNO = '+IntToStr(VpaSEqRetorno));
  end;
...
{******************************************************************************}
procedure TRBFuncoesOrdemProducao.CalculaTotaisRevisaoTerceiro(VpaDRevisao: TRBDRevisaoFracaoFaccionista);
var
  VpfLaco : Integer;
  VpfDTerceiro : TRBDRevisaoFracaoTerceiroFaccionista;
begin
  VpaDRevisao.QtdRevisado := 0;
  VpaDRevisao.QtdDefeito := 0;
  for VpfLaco := 0 to VpaDRevisao.Terceiros.Count - 1 do
  begin
    VpfDTerceiro := TRBDRevisaoFracaoTerceiroFaccionista(VpaDRevisao.Terceiros.Items[VpfLaco]);
    VpaDRevisao.QtdRevisado := VpaDRevisao.QtdRevisado + VpfDTerceiro.QtdRevisado;
    VpaDRevisao.QtdDefeito := VpaDRevisao.QtdDefeito + VpfDTerceiro.QtdDefeito;
  end;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.RValorEstagio(VpaCodEstagio: Integer): double;
begin
  AdicionaSQLAbreTabela(OrdTabela,'Select VALHOR from ESTAGIOPRODUCAO ' +
                                  ' Where CODEST = ' +IntToStr(VpaCodEstagio));
  Result := OrdTabela.FieldByName('VALHOR').AsFloat;
  OrdTabela.Close;
end;
...
begin
  ...
  if config.PermitirEnviarFracaoMaisde1VezparaoMesmoFaccionista then
    OrdAux.sql.add(' and DATFINALIZACAO IS NULL ');
  ...
  OrdAux.open;
  VpaDRetornoFaccionista.QtdFaltante := 0;
  while not ordaux.eof do
  begin
    ...
    VpaDRetornoFaccionista.QtdFaltante := VpaDRetornoFaccionista.QtdFaltante +
                                          OrdAux.FieldByname('QTDENVIADO').AsFloat -
...
function TRBFuncoesOrdemProducao.AtualizaValPesoMateriaPrimaPlanoCorte(VpaDPlanoCorte: TRBDPlanoCorteCorpo; VpaDMateriaPrima : TRBDProduto; VpaPerDiferenca: Double): string;
var
  VpfLacoProduto, VpfLacoFracao : Integer;
  VpfDItem : TRBDPlanoCorteItem;
  VpfDFracao : TRBDPlanoCorteFracao;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        VpfDFracao.PesProduto := VpfDFracao.PesProduto + ((VpfDFracao.PesProduto * VpaPerDiferenca)/100) ;
        OrdCadastro.Edit;
        OrdCadastro.FieldByName('PESPRODUTO').AsFloat := VpfDFracao.PesProduto;
        OrdCadastro.FieldByName('VALCUSTOTOTAL').AsFloat := VpfDFracao.PesProduto * VpaDMateriaPrima.VlrCusto;
        OrdCadastro.Post;
        result := OrdCadastro.AMensagemErroGravacao;
        if OrdCadastro.AErronaGravacao then
          exit;
...
begin
  ...
  if VpfConsumo.Count > 0 then
    VpfResultado := GravaDImpressaoConsumoFracao(VpaCodFilial,VpaSeqOrdem,VpaSeqFracao,RProdutoFracaoOP(VpaCodFilial,VpaSeqOrdem,VpaSeqFracao),RCorFracaoOP(VpaCodFilial,VpaSeqOrdem,VpaSeqFracao),VpfConsumo);
  ...
  begin
    ...
    VpfResultado := GravaDImpressaoConsumoFracao(OrdTabela.FieldByName('CODFILIAL').AsInteger,OrdTabela.FieldByName('SEQORDEM').AsInteger,OrdTabela.FieldByName('SEQFRACAO').AsInteger,OrdTabela.FieldByName('SEQPRODUTO').AsInteger,OrdTabela.FieldByName('CODCOR').AsInteger, VpfConsumo);
    if VpfResultado <> '' then
      aviso(VpfResultado);
    FreeTObjectsList(VpfConsumo);
    VpfConsumo.free;
...
begin
  ...
  begin
    ...
    VpfDNovaOrdemProducao.SeqFracaoDisponivelReimportacao := RSeqFracaoDisponivel(VpaCodFilial,VpaSeqOrdem);
...
{******************************************************************************}
procedure TRBFuncoesOrdemProducao.RenumeraFracao(VpaDOrdemProducao: TRBDOrdemProducao; VpaSeqFracao: Integer);
var
  VpfDFracaoARenumerar : TRBDFracaoOrdemProducao;
begin
  VpfDFracaoARenumerar := RFracaoOP(VpaDOrdemProducao,VpaSeqFracao);
  if VpfDFracaoARenumerar <> nil then
  begin
    AlteraSeqFracaoPai(VpaDOrdemProducao.Fracoes,VpfDFracaoARenumerar.SeqFracao,VpaDOrdemProducao.SeqFracaoDisponivelReimportacao);
    VpfDFracaoARenumerar.SeqFracao := VpaDOrdemProducao.SeqFracaoDisponivelReimportacao;
    inc(VpaDOrdemProducao.SeqFracaoDisponivelReimportacao);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if VpfDFracaoAtual.SeqFracao <> VpaDFracaoNova.SeqFracao then
      begin
        //nas fracoes geradas novas provavelmente existe uma fração com o mesmo numero da fração que foi encontrado nas fraçoes atuais
        // e que nao é o mesmo produto é necessario dar um novo numero para essa fração, arrumar os link das fraçoes pais para não dar conflito
        RenumeraFracao(VpaDOrdemProducaoNova,VpfDFracaoAtual.SeqFracao);
      end;
  ...
  begin
    VpfNovoSeqFracao := VpaDOrdemProducaoNova.SeqFracaoDisponivelReimportacao;
    inc(VpaDOrdemProducaoNova.SeqFracaoDisponivelReimportacao);
