...
uses
  ...
  EditorImagem, ImgList, numericos, UnProdutos, DBKeyViolation, FMTBcd, SqlExpr;
...
type
  TFAdicionaProdFilial = class(TFormularioPermissao)
    ...
    LblQtdPed: TLabel;
    Label5: TLabel;
    EValorCusto: Tnumerico;
    Label2: TLabel;
    BAdicionarTodos: TBitBtn;
...
begin
  ...
  Listar := True;
  FunProdutos.VerificaMascaraClaPro; // Verifica se existe máscara para a classificação;
  CadProdutos.open;
...
begin
  ...
  if TClassificacao(NoSelecao.Data).tipo = 'CL' then
    FunProdutos.InseriProdutoClassificacaoFilial( TClassificacao(NoSelecao.Data).Codigo,VprEmpFil,VprCodEmp,
                                                 NQtdMin.AValor, NQtdPed.AValor, EValorCusto.AValor, true)
  else
    FunProdutos.InseriProdutoClassificacaoFilial( TClassificacao(NoSelecao.Data).Sequencial,VprEmpFil,VprCodEmp,
                                                 NQtdMin.AValor, NQtdPed.AValor, EValorCusto.AValor, false);
  TTempo.Fecha;
end;
...
procedure TFAdicionaProdFilial.BAdicionarTodosClick(Sender: TObject);
begin
  FunProdutos.AdicionaProdutosFilialAtiva;
end;
