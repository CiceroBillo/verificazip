...
uses
  ...
  AImportacaoDados in 'AImportacaoDados.pas' {FImportacaoDados},
  AMostraEstoqueChapas in '..\mestoque\AMostraEstoqueChapas.pas' {FMostraEstoqueChapas},
  UnImportacaoDados in 'UnImportacaoDados.pas',
  UnDRave in '..\magerais\UnDRave.pas',
  UnProgramador1 in '..\magerais\UnProgramador1.pas',
  AExportacaoDados in 'AExportacaoDados.pas' {FExportacaoDados},
  UnExportacaoDados in 'UnExportacaoDados.pas',
  AConsultaCodigoRepresentante in 'AConsultaCodigoRepresentante.pas' {FConsultaCodigoRepresentante},
  ANovaTabelaImportacaoRepresentante in 'ANovaTabelaImportacaoRepresentante.pas' {FNovaTabelaImportacaoRepresentante},
  AMostraErrosExportacaoDados in 'AMostraErrosExportacaoDados.pas' {FMostraErrosExportacaoDados},
  AMostraPedidosEClientesExportados in 'AMostraPedidosEClientesExportados.pas' {FMostraPedidosEClientesExportados},
  ...
  AVersaoDesatualizada in 'AVersaoDesatualizada.pas' {FVersaoDesatualizada},
...
Var
  ...
  if ParamCount > 2 then  // verifica se a parametros da base de dados
  begin
    varia.usuario  := ParamStr(2);
    varia.senha := ParamStr(3);
  end;
  ...
  begin
    ...
    begin
      ...
      if FVersaoDesatualizada.VersaoSistemaPedidosValida then
      begin
        FVersaoDesatualizada.free;
        if Sistema.RDataUltimaImportacao <= montadata(1,1,1900) then
        begin
          varia.senha := 'MORRO';
          varia.usuario  := 'RAFAEL';
        end;

        FAbertura := TFAbertura.create(application);
        if ( varia.usuario<> '') and (Varia.senha<> '') then    // caso a senha e ususario venham como parametro
        begin
          if FAbertura.VerificaSenha(Uppercase(varia.usuario),Uppercase(Varia.senha), ParamStr(4), ParamStr(5)) then // verifica a senha
             FAbertura.close
          else
             FAbertura.ShowModal;
          end
          ...
          if Varia.StatusAbertura <> 'CANCELADO' then
            if (varia.CodVendedorSistemaPedidos = 0) and (varia.DatUltimaImportacao < MontaData(1,1,1900)) then
              if not FPrincipal.ConfiguraBaseInicialRepresentante then
