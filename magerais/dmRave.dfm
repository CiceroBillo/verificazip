object dtRave: TdtRave
  ...
  object Principal: TSQL
    ...
    ASqlQuery.SQL.Strings = (
        ...
        'Select COR.SEQPEDIDO, COR.CODFILIALFATURAMENTO, COR.CODFILIAL, C' +
        'OR.SEQPEDIDO, COR.DATPEDIDO, COR.HORPEDIDO, COR.DATPREVISTA, COR' +
        '.NOMCONTATO,  COR.CODCONDICAOPAGAMENTO, COR.VALTOTAL, COR.PERDES' +
        'CONTO, COR.VALDESCONTO, COR.DESOBSERVACAO,  COR.VALFRETE, COR.TI' +
        'PFRETE,  CLI.C_NOM_CLI, CLI.C_END_CLI, CLI.I_NUM_END, CLI.C_BAI_' +
        'CLI, CLI.C_CEP_CLI, CLI.C_FO1_CLI,  CLI.C_FON_FAX, CLI.C_END_ELE' +
        ',CLI.I_COD_CLI, CLI.C_CGC_CLI, CLI.C_INS_CLI,  CLI.C_CID_CLI, CL' +
        'I.C_EST_CLI,CLI.C_COM_END, CLI.C_FON_COM,  COM.NOMCOMPRADOR ,  U' +
        'SU.C_NOM_USU,  TRA.C_NOM_CLI C_NOM_TRA,  FRM.C_NOM_FRM,  PAG.C_N' +
        'OM_PAG  from PEDIDOCOMPRACORPO COR, CADCLIENTES CLI, COMPRADOR C' +
        'OM,  CADUSUARIOS USU, CADCLIENTES TRA, CADFORMASPAGAMENTO FRM, C' +
        'ADCONDICOESPAGTO PAG  Where COR.CODCLIENTE = CLI.I_COD_CLI  AND ' +
        'COR.CODCOMPRADOR = COM.CODCOMPRADOR  AND COR.CODUSUARIO = USU.I_' +
        'COD_USU  AND   COR.CODTRANSPORTADORA = TRA.I_COD_CLI (+)  AND   ' +
        'COR.CODFORMAPAGAMENTO = FRM.I_COD_FRM (+)  AND   COR.CODCONDICAO' +
        'PAGAMENTO = PAG.I_COD_PAG (+)  and COR.CODFILIAL = 11 and COR.SE' +
        'QPEDIDO = 2177')
  ...
  object Item: TSQL
    ...
    ASqlQuery.SQL.Strings = (
        ...
        'select PRO.C_COD_PRO, PRO.C_NOM_PRO, PRO.L_DES_TEC,  ITE.QTDPROD' +
        'UTO,ITE.VALUNITARIO,ITE.VALTOTAL, ITE.CODCOR, ITE.CODTAMANHO,  I' +
        'TE.DESUM, ITE.DESREFERENCIAFORNECEDOR, ITE.QTDCHAPA, ITE.COMCHAP' +
        'A, ITE.LARCHAPA, ITE.PERIPI, ITE.VALIPI, COR.NOM_COR,  TAM.NOMTA' +
        'MANHO  from  PEDIDOCOMPRAITEM ITE, CADPRODUTOS PRO, COR COR, TAM' +
        'ANHO TAM  WHERE ITE.SEQPRODUTO = PRO.I_SEQ_PRO  and ITE.CODFILIA' +
        'L = 11 and ITE.SEQPEDIDO = 2177 AND   ITE.CODCOR = COR.COD_COR (' +
        '+)  AND   ITE.CODTAMANHO = TAM.CODTAMANHO (+)  ORDER BY SEQITEM ')
    ASqlQuery.SQLConnection = FPrincipal.BaseDados
    SQL.Strings = (
